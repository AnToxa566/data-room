...
| Pending-share resolution | Resolved immediately at share-creation time if the invited email already matches a registered `User`; otherwise deferred to that user's next Google login, gated on the provider reporting the email verified | Trust any claimed email as an access key on its own | An email typed by the share's creator is not proof of ownership by itself — but a `User` row existing for that address means Google already verified it at some past login, so matching against an *existing account* at creation time trusts Google's prior verification, not the creator's input. An invited address with no account yet still can't get `granteeUserId` until its owner completes their own verified login — first drafted without the creation-time half, which meant an already-registered invitee stayed unresolved (and invisible in their own "Shared with me") until they happened to log out and back in; see the incident this amended |
